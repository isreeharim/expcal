...
export function PrintableInvoice({ project, config, isPrintMode = false }: PrintableInvoiceProps) {
  ...
  return (
    <div
      ...
      className={cn(
        'w-full bg-white text-zinc-900 font-sans flex flex-col',
        isPrintMode
    ...
    >
      ...
      <div className="flex flex-row border-b-2 border-zinc-300 flex-shrink-0">
        ...
        {/* Right: Invoice meta */}
        <div className="px-8 py-7 flex flex-col justify-center items-end text-right border-l-2 border-zinc-200 bg-zinc-50/70 min-w-[190px]">
      ...
      {/* ── BILLED TO ── only when client name/company is entered */}
      {config.showClient && hasClient && (
        <div className="px-8 py-5 border-b-2 border-zinc-200 bg-zinc-50/40 flex-shrink-0">
          ...
          {config.clientPhone && (
            <p className="text-xs text-zinc-500">{config.clientPhone}</p>
          )}
          {config.clientTaxId && (
      ...
      {/* ── ITEMS TABLE ── */}
      <div className="px-8 pt-7 pb-6 flex-shrink-0">
      ...
      {config.showSignature && (config.signatoryName || config.signatoryTitle) && (
        <div className="mx-8 pt-5 pb-6 border-t-2 border-zinc-200 flex justify-end flex-shrink-0">
      ...
      {/* ── FOOTER ── */}
      <div className="border-t-2 border-zinc-200 px-8 py-4 bg-zinc-50/60 flex justify-between items-center text-[10px] text-zinc-400 flex-shrink-0">
